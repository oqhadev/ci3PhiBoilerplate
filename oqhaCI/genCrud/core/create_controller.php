...
$string .= "\n\n    public function index()
    {
        ...
        if (\$q <> '') {
            ...
            \$config['first_url'] = base_url() . 'index.php/$argv[4]/$c_url/index.html?q=' . urlencode(\$q);
        } else {
            \$config['base_url'] = base_url() . 'index.php/$argv[4]/$c_url/index/';
            \$config['first_url'] = base_url() . 'index.php/$argv[4]/$c_url/index/';
        }
        ...
        \$this->template->load('admin','$argv[4]/$c_url/$v_list', \$data);
    }";
...
$string .="\n\n    public function index()
    {
        \$this->template->load('admin','$argv[4]/$c_url/$v_list');
    } 
 ...
 $string .="   public function create() 
    {
        \$data = array(
            'button' => 'Create',
            'action' => site_url(".''."'$argv[4]/$c_url/create_action'),";
foreach ($all as $row) {
...
$string .= "\n\t    );
    ...
    public function update(\$id) 
    {
        ...
        if (\$row) {
            \$data = array(
                'button' => 'Update',
                'action' => site_url(".''."'$argv[4]/$c_url/update_action'),";
foreach ($all as $row) {
